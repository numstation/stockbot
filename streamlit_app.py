        ...
        if not stock_input.strip():
            ...
            with st.spinner("正在分析股票數據..."):
                ...
                if result['success']:
                    ...
                    # Compact Tables (Technical + Fundamental) & Copy Report for Gems
                    ...
                    if details:
                        ...
                        adx_note = f"Slope: {adx_slope_val:.2f}"
                        ...
                        rvol_note = "High vol" if rvol_val > 2.0 else "—"
                        mfi_note = "Overbought" if mfi_val > 80 else "Oversold" if mfi_val < 20 else "—"
                        tech_data = [
                            ("RSI", f"{rsi_val:.2f}", rsi_note),
                            ("ADX", f"{adx_val:.2f}", adx_note),
                            ("PDI", f"{pdi_val:.2f}", "—"),
                            ("MDI", f"{mdi_val:.2f}", "—"),
                            ("Gap", f"{pdi_mdi_gap:.2f}", "—"),
                            ("ATR", f"{atr_val:.2f}", "—"),
                            ("RVOL", f"{rvol_val:.2f}", rvol_note),
                            ("MFI", f"{mfi_val:.2f}", mfi_note),
                            ("SMA 50", sma_50_str, "—"),
                            ("SMA 200", sma_200_str, "—"),
                        ]
                        tech_df = pd.DataFrame(tech_data, columns=["Indicator", "Value", "Status/Note"])
                        st.markdown("#### Technical Data")
                        st.dataframe(tech_df, use_container_width=True, hide_index=True)
                        
                        # Table B: Fundamental & Risk
                        ...
                        pe_trail_str = f"{trailing_pe:.2f}" if trailing_pe is not None else "N/A"
                        pe_fwd_str = f"{forward_pe:.2f}" if forward_pe is not None else "N/A"
                        debt_str = f"{debt_to_equity:.2f}" if debt_to_equity is not None else "N/A"
                        profit_str = f"{profit_margins*100:.2f}%" if profit_margins is not None else "N/A"
                        next_earn_str = next_earnings if next_earnings else "N/A"
                        fund_data = [
                            ("Price", f"{current_price_val:.2f}"),
                            ("PE (Trail)", pe_trail_str),
                            ("PE (Fwd)", pe_fwd_str),
                            ("Debt/Equity", debt_str),
                            ("Profit Margin", profit_str),
                            ("Next Earnings Date", next_earn_str),
                        ]
                        fund_df = pd.DataFrame(fund_data, columns=["Metric", "Value"])
                        st.markdown("#### Fundamental & Risk")
                        st.dataframe(fund_df, use_container_width=True, hide_index=True)
                        
                        # Copy Report payload (includes hidden Robot Analysis)
                        ...
                        report_ts = datetime.now(hk_tz).strftime("%Y-%m-%d %H:%M:%S")
                        ...
                        commentary = result.get('analyst_commentary') or result.get('market_analysis') or ''
                        strategy_type = signal.get('strategy_type', 'none') if signal else 'none'
                        table_a_text = "\n".join([f"  {r[0]}: {r[1]}  ({r[2]})" for r in tech_data])
                        table_b_text = "\n".join([f"  {r[0]}: {r[1]}" for r in fund_data])
                        robot_section = f"""Signal: {signal_advice}
Reason: {signal_reason if signal_reason else 'No additional details'}
Strategy: {strategy_type}
{commentary}"""
                        copy_text = f"""Report Generated: {report_ts} (HKT)
...
{robot_section}"""
                        st.download_button(
                            label="📋 **One-Click: Copy Report for Gems**",
                            data=copy_text,
                        ...
                        # (Company Health / Analyst Report / Signal badge removed from UI; data still in copy_text)
                    # (Signal badge, Analyst Report, Strike price hidden from UI; included in Copy Report for Gems)
                else:
